...
public class King extends Piece{
	...
	private int x,y; // Variables adicionales para la clase King para hacer un seguimiento de la posición del rey
	...
	// Función de movimiento para King, sobrescrita desde Piece
	public ArrayList<Cell> move(Cell state[][],int x,int y)
	{
		// El rey solo puede moverse un paso. Por lo tanto, se han considerado todas las 8 celdas adyacentes.
		possiblemoves.clear();
	...
	// Función para comprobar si el rey está en peligro
	// Comprueba si hay alguna pieza de color opuesto que pueda atacar al rey para una configuración dada del tablero
	public boolean isindanger(Cell state[][])
    {
		// Comprobar el ataque desde la izquierda, derecha, arriba y abajo
    	for(int i=x+1;i<8;i++)
    	{
    	...
    	for(int i=x-1;i>=0;i--)
    	{
    		...
    		{
    			if ((state[i][y].getpiece() instanceof Rook) || (state[i][y].getpiece() instanceof
 Queen))
    				return true;
    			else
    				break;
    	...
    	//Comprobar si hay un ataque desde una dirección diagonal.
		...
		//Verificar si hay un ataque desde el Caballo de color opuesto.
		...
		//Comprobar si hay un ataque desde el peón de color opuesto.
